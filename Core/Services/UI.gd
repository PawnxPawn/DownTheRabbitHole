...
const _UI_SCENES_PRELOAD: Dictionary = {
	Uis.GAME_SETTINGS: preload("uid://dw0hrwf0gw2hw")
}
...
func show_ui(ui: Uis) -> void:
	if not _ui_manager_check() or _active_uis.has(ui): return
...
func hide_ui(ui:Uis) -> void:
	if not _ui_manager_check(): return
...
func _load_ui_scene(ui:Uis) -> void:
	if _loaded_uis.has(ui): return
	
	var instance = _UI_SCENES_PRELOAD[ui].instantiate()
